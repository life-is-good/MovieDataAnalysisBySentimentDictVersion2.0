# -*- coding: utf-8 -*-
from __future__ import absolute_import, unicode_literals	#允许绝对引用,禁用implicit relative import, 但并不会禁掉explicit relative import.在py2下，默认采用unicode编码
__version__ = '0.36.2'
__license__ = 'MIT'

import re				#模块re：包含正则表达式的所有方法
import os				#模块os：提供了一个统一的操作系统接口函数
import sys				#模块sys：包含系统对应的功能
import time				#模块time：提供操作时间的函数
import logging			#模块logging：提供日志处理方式
import marshal			#模块marshal：实习序列化
import tempfile			#模块tempfile：临时文件夹操作 
import threading		#模块threading：线程操作
from math import log	#函数log：模块math里的log函数
from hashlib import md5	#函数md5：模块hashlib里的md5jiami方式 
from ._compat import *
